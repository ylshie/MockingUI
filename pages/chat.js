...
const colmap = [ styles.single, styles.cols, styles.cols3 ];
...
//  Avatar
    ...
    const [model,  setModel]    = useState("meme.vrm"); //hemo-2.vrm , demo-5.vrm
    ...
    */
    ...
    return (
        ...
        <div className={styles.threeback}>
            <img src="7-11_back.png" width={"100%"} height={"100%"}/>
        </div>
        <div className={(is711)? styles.inner711: styles.inner}>
            {
            (header)
            ...
            {/*
            <div className={(test)? ( is711 ? ((role=="master")? styles.cols3: styles.single): styles.cols): styles.single}>
            */}
            <div className={colmap[cols]}>
                {
                (test)
                ? <div className={styles.threeframe}>
                    <Avatar file={model} action={action} speak={speak} pose={1}/>
                        ...
                        ? (showCS)
                            ? <div className={styles.customframe}>
                                <div className={styles.custominner}>
                                    <div onClick={OnCloseCS}>
                                        <img src="7-11_hang.png" width={"110px"} height={"50px"}/>
                                    </div>
                                </div>
                            </div>
                            : <div className={styles.customframe}>
                                <div className={styles.custominner}>
                                    <div onClick={()=> setShowQA(true)}>
                                        <img src="7-11_qa.png" width={"110px"} height={"50px"}/>
                                    </div>
                                    <div onClick={onCustomService}>
                                        <img src="7-11_cs.png" width={"110px"} height={"50px"}/>
                                    </div>
                                </div>
                            </div>
                        : <></>
                    }
                    {
                        ...
                        ? <div className={styles.qapanel}>
                            <div>
                                <button onClick={()=> setShowQA(false)}>關閉</button>
                            </div>
                            <iframe className={styles.qaframe} title="常見問題"
                                src="https://www.family.com.tw/marketing/faq#consumer"
                            >
                            </iframe>
                          </div>
                        : <></>
                    ...
                    ?   
                        <div>
                            ...
                            ?   <>客服呼叫:
                                {
                                    (rooms != null)
                                    ? rooms.map((room, index) => <div className={styles.room} key={index} onClick={()=>selectRoom(index)} value={index}>
                                                                    店 {index}
                                                                </div>)
                                    : <>無</>
                                }
                            ...
                            }
                            {
                                ...
                                ? <div className={styles.videoframe}>
                                    <div id={(role=="master")? styles.remotevideo: styles.remotehidden} ref={refRemote}></div>
                                    <div id={styles.localvideo}  ref={refLocal}></div>
                                </div>
                                : <></>
                            }
                        </div>
                ...
                {
                    ((is711 && role == "slave") || mode=="chat")
                    ? <></>
                    : <main className={styles.main}>
                        <Conversation result={result} saying={saying}/>
                    </main>
                }
            </div>
            {
...
export async function getServerSideProps({params,req,res,query,preview,previewData,resolvedUrl,locale,locales,defaultLocale}) {
    ...
    const dospeak = test;
    // Layout
    const cols  = 0; //(test)? ( is711 ? ((role=="master")? 2: 0): 1): 0;
    const header = false;   //test && mode != "mac";
