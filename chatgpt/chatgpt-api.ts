...
const MODEL_GPT3 = 'text-davinci-003'
//const MODEL_CHAT = 'gpt-3.5-turbo'
const MODEL_CHAT = 'gpt-4'

const USER_LABEL_DEFAULT = 'user'
const ASSISTANT_LABEL_DEFAULT = 'assistant' //'ChatGPT'
...
export class ChatGPTAPI {
  ...
  constructor(opts: {
    ...
    this._completionParams = {
      model: (this._mode)? MODEL_CHAT: MODEL_GPT3,  //CHATGPT_MODEL,
      temperature: 0.7,
  ...
  async sendMessage(
    ...
    //console.log("_upsertMessage")
    await this._upsertMessage(message)

    //console.log("_buildPrompt")
    const { prompt, messages, maxTokens } = await this._buildPrompt(text, opts)
    ...
    const responseP = new Promise<types.ChatMessage>(
      async (resolve, reject) => {
        const url = (this._mode)?`${this._apiBaseUrl}/v1/chat/completions`: `${this._apiBaseUrl}/v1/completions`;
        const headers = {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this._apiKey}`,
          'OpenAI-Organization': "org-gboaIHUvqWAGlzoXh4UcqrwC"
        }
        const body = (this._mode) 
                     ? {max_tokens: maxTokens, ...this._completionParams, messages, stream}
                     : {max_tokens: maxTokens, ...this._completionParams, prompt,   stream};
        ...
        if (stream) {
            ...
            onMessage: (data: string) => {
              ...
              try {
                ...
                if (response?.id && response?.choices?.length) {
                  ...
                  if (this._mode) {
                      result.text += (response.choices[0].delta.content) ? response.choices[0].delta.content: "";
                  } else {
          ...
          try {
            ...
            const response: types.openai.CompletionResponse = (await res.json()) as types.openai.CompletionResponse;
            if (this._debug) {
              console.log(response)
            }

            result.id = response.id
            result.text = (this._mode)? response.choices[0].message.content :response.choices[0].text.trim()
...
//Current date: ${currentDate}\n\n`
    ...
    let promptBody = ''
    let prompt: string
    let messages: [{}] = new Array() as [{}];
    let nextX: [{}]   = new Array() as [{}];
    let prefixX: [{}] = new Array() as [{}];
    let numTokens: number
    const x = {role: "system", content: promptPrefix}
    const y = {role: this._userLabel, content: message}

    prefixX.push(x)
    nextX.push(y)
    //nextX[0]["role"]      = this._userLabel; 
    //nextX[0]["content"]   = message;
    //prefixX[0]["content"] = promptPrefix;
    ...
    do {
        ...
        prompt = nextPrompt
        messages = prefixX.concat(nextX) as [{}];
        ...
        const parentX = new Array() as [{}];
        parentX.push({role: parentMessageRoleDesc, content: parentMessage.text})

        nextX = parentX.concat(nextX) as [{}];
    ...
    const maxTokens = 600; //2400;
  
    return { prompt, messages, maxTokens }
  }
  /*
